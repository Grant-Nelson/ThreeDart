...
class Board extends ThreeDart.Entity {
  ...
  Board(ThreeDart.ThreeDart td, this._game):
    ...
    ThreeDart.Entity edges = this._edges = new ThreeDart.Entity();
    this.children.add(edges);
    edges.children.add(new Edge(td, this, 0.0, 0.0, 0.0, 0));
    edges.children.add(new Edge(td, this, 8.0, 0.0, Math.PI_2, 1));
    edges.children.add(new Edge(td, this, 8.0, 8.0, Math.PI, 2));
    edges.children.add(new Edge(td, this, 0.0, 8.0, Math.PI3_2, 3));
  ...
  void setHighlights(List<game.Movement> movements) {
    for (game.Movement movement in movements) {
      ...
      var otherSource = movement.otherSource;
      if (otherSource != null) {
        Piece? piece = this.pieceAt(otherSource);
