...
class _objReader {
  ...
  Future processLines(List<String> lines, {bool strict: false, String dir: ''}) async {
    for (int i = 0; i < lines.length; ++i) {
      ...
      if ((progress != null) && ((i % 1000) == 0))
        progress.emit(new ProgressEventArgs(this, i*100.0/lines.length, false));
  ...
  void _startShape() {
    ...
    if (entity == null || shape == null || (!shape.vertices.isEmpty)) {
      this._shape = shape = new Shapes.Shape();
      this._entity = entity = new Core.Entity(shape: shape);
  ...
  Shapes.Vertex _addVertex(String vertexStr) {
    ...
    Math.Point2 txt2D = Math.Point2.zero;
    ...
    Math.Vector3 norm = Math.Vector3.zero;
