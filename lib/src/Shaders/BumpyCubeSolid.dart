...
/// A shader for rendering bumpy cube map solid color light.
class BumpyCubeSolid extends Shader {

  /// The name for this shader.
  static const String defaultName = "Bumpy Cube Solid";
  ...
  static String _fragmentSource =
      ...
      "vec3 bumpyNormal()                                            \n"+
      "{                                                             \n"+
      "   vec3 color = textureCube(bumpTxt, txtCube).rgb;            \n"+
  ...
  factory BumpyCubeSolid.cached(Core.RenderState state) {
    BumpyCubeSolid shader = state.shader(defaultName);
    if (shader == null) {
      shader = new BumpyCubeSolid(state.gl);
  ...
  BumpyCubeSolid(WebGL.RenderingContext gl): super(gl, defaultName) {
    ...
    this._bumpTxt        = this.uniforms["bumpTxt"] as UniformSamplerCube;
  ...
  /// The normal distortion cube texture of the object.
  set bumpTexture(Textures.TextureCube txt) =>
    this._bumpTxt.setTextureCube(txt);

  /// Sets the material's colors.
  void setMaterial(Materials.BumpyCubeSolid material) {
