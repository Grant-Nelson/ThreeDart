...
class MaterialLight extends Shader {
  ...
  Attribute? _txt2DAttr   = null;
  Attribute? _txtCubeAttr = null;
  Attribute? _bendAttr    = null;

  UniformMat4? _objMat         = null;
  UniformMat4? _viewObjMat     = null;
  UniformMat4? _viewMat        = null;
  UniformMat4? _projViewObjMat = null;
  UniformMat4? _projViewMat    = null;
  UniformMat4? _invViewMat     = null;
  UniformMat3? _txt2DMat       = null;
  UniformMat4? _txtCubeMat     = null;
  UniformMat4? _colorMat       = null;

  Uniform1i? _bendMatCount = null;
  List<UniformMat4?> _bendMatrices = [];

  Uniform3f? _emissionClr = null;
  UniformSampler2D? _emission2D = null;
  UniformSamplerCube? _emissionCube = null;

  Uniform3f? _ambientClr = null;
  UniformSampler2D? _ambient2D = null;
  UniformSamplerCube? _ambientCube = null;

  Uniform3f? _diffuseClr = null;
  UniformSampler2D? _diffuse2D = null;
  UniformSamplerCube? _diffuseCube = null;

  Uniform3f? _invDiffuseClr = null;
  UniformSampler2D? _invDiffuse2D = null;
  UniformSamplerCube? _invDiffuseCube = null;

  Uniform3f? _specularClr = null;
  UniformSampler2D? _specular2D = null;
  UniformSamplerCube? _specularCube = null;
  Uniform1f? _shininess = null;

  UniformSampler2D? _bump2D = null;
  UniformSamplerCube? _bumpCube = null;

  UniformSamplerCube? _envSampler = null;

  Uniform3f? _reflectClr = null;
  UniformSampler2D? _reflect2D = null;
  UniformSamplerCube? _reflectCube = null;

  Uniform1f? _refraction = null;
  Uniform3f? _refractClr = null;
  UniformSampler2D? _refract2D = null;
  UniformSamplerCube? _refractCube = null;

  Uniform1f? _alpha = null;
  UniformSampler2D? _alpha2D = null;
  UniformSamplerCube? _alphaCube = null;
  ...
  Uniform4f? _fogClr   = null;
  Uniform1f? _fogStop  = null;
  Uniform1f? _fogWidth = null;
  ...
  MaterialLight(this._cfg,  WebGL.RenderingContext2 gl): super(gl, _cfg.name) {
    ...
    if (this._cfg.emission.hasSolid)
    ...
    if (this._cfg.invDiffuse.hasSolid)
    ...
    if (this._cfg.specular.hasAny) {
      ...
      if (this._cfg.specular.hasSolid)
    ...
    if (this._cfg.environmental) {
      ...
      if (this._cfg.reflection.hasSolid)
      ...
      if (this._cfg.refraction.hasAny) {
        ...
        if (this._cfg.refraction.hasSolid)
    ...
    if (this._cfg.lights) {
      ...
      if (this._cfg.barLights.isNotEmpty) {
        ...
        this._barLights      = {};
        for (BarLightConfig light in this._cfg.barLights) {
          final int configID = light.configID;
          final String name = light.toString();
          List<UniformBarLight> lights = [];
          for (int i = 0; i < light.lightCount; ++i) {
            ...
            Uniform1f? att0 = null, att1 = null, att2 = null;
      ...
      if (this._cfg.dirLights.isNotEmpty) {
        ...
        this._dirLights      = {};
        for (DirectionalLightConfig light in this._cfg.dirLights) {
          final int configID = light.configID;
          final String name = light.toString();
          List<UniformDirectionalLight> lights = [];
          for (int i = 0; i < light.lightCount; ++i) {
            Uniform3f? objUp = null, objRight = null, objDir = null;
            if (light.hasTexture) {
      ...
      if (this._cfg.pointLights.isNotEmpty) {
        ...
        this._pointLights      = {};
        for (PointLightConfig light in this._cfg.pointLights) {
          final int configID = light.configID;
          final String name = light.toString();
          List<UniformPointLight> lights = [];
          for (int i = 0; i < light.lightCount; ++i) {
            ...
            Uniform3f? color   = this.uniforms.required("${name}s[$i].color")   as Uniform3f;
            UniformMat3? invViewRotMat = null;
            if (light.hasTexture)
              invViewRotMat = this.uniforms.required("${name}s[$i].invViewRotMat") as UniformMat3;
            Uniform4f? shadowAdj = null;
            UniformSamplerCube? txt = null, shadow = null;
            if (light.colorTexture)
      ...
      if (this._cfg.spotLights.isNotEmpty) {
        ...
        this._spotLights      = {};
        for (SpotLightConfig light in this._cfg.spotLights) {
          final int configID = light.configID;
          final String name = light.toString();
          List<UniformSpotLight> lights = [];
          for (int i = 0; i < light.lightCount; ++i) {
            ...
            Uniform3f? viewPnt = this.uniforms.required("${name}s[$i].viewPnt") as Uniform3f;
            Uniform3f? color   = this.uniforms.required("${name}s[$i].color")   as Uniform3f;
            Uniform3f? objUp = null, objRight = null;
            Uniform1f? tuScalar = null, tvScalar = null;
            if (light.hasTexture) {
              objUp    = this.uniforms.required("${name}s[$i].objUp")    as Uniform3f;
              objRight = this.uniforms.required("${name}s[$i].objRight") as Uniform3f;
            ...
            if (light.shadowTexture)
              shadowAdj = this.uniforms.required("${name}s[$i].shadowAdj") as Uniform4f;
            Uniform1f? cutoff = null, coneAngle = null;
            if (light.hasCutOff) {
            ...
            UniformSampler2D? txt = null, shadow = null;
            if (light.colorTexture)
              txt = this.uniforms.required("${name}sTexture2D$i") as UniformSampler2D;
            if (light.shadowTexture)
  ...
  Math.Matrix4 get projectViewMatrix => this._projViewMat?.getMatrix4() ?? Math.Matrix4.identity;
  set projectViewMatrix(Math.Matrix4 mat) => this._projViewMat?.setMatrix4(mat);

  /// The inverse view matrix.
  Math.Matrix4 get inverseViewMatrix => this._invViewMat?.getMatrix4() ?? Math.Matrix4.identity;
  set inverseViewMatrix(Math.Matrix4 mat) => this._invViewMat?.setMatrix4(mat);

  /// The 2D texture modification matrix.
  Math.Matrix3 get texture2DMatrix => this._txt2DMat?.getMatrix3() ?? Math.Matrix3.identity;
  set texture2DMatrix(Math.Matrix3 mat) => this._txt2DMat?.setMatrix3(mat);

  /// The cube texture modification matrix.
  Math.Matrix4 get textureCubeMatrix => this._txtCubeMat?.getMatrix4() ?? Math.Matrix4.identity;
  set textureCubeMatrix(Math.Matrix4 mat) => this._txtCubeMat?.setMatrix4(mat);

  /// The color modification matrix.
  Math.Matrix4 get colorMatrix => this._colorMat?.getMatrix4() ?? Math.Matrix4.identity;
  ...
  Math.Matrix4 getBendMatrix(int index) => this._bendMatrices[index]?.getMatrix4() ?? Math.Matrix4.identity;

  /// The emission color scalar of the object.
  Math.Color3 get emissionColor => this._emissionClr?.getColor3() ?? Math.Color3.white();
  ...
  Math.Color3 get ambientColor => this._ambientClr?.getColor3() ?? Math.Color3.white();
  ...
  Math.Color3 get diffuseColor => this._diffuseClr?.getColor3() ?? Math.Color3.white();
  ...
  Math.Color3 get invDiffuseColor => this._invDiffuseClr?.getColor3() ?? Math.Color3.white();
  ...
  Math.Color3 get specularColor => this._specularClr?.getColor3() ?? Math.Color3.white();
  ...
  Math.Color3 get reflectionColor => this._reflectClr?.getColor3() ?? Math.Color3.white();
  ...
  Math.Color3 get refractionColor => this._refractClr?.getColor3() ?? Math.Color3.white();
  ...
  Math.Color4 get fogColor => this._fogClr?.getColor4() ?? Math.Color4.white();
