...
class Bar implements Light {
  ...
  void update(Core.RenderState state) {
    this._startMatrix = this._startMover?.update(state, this) ?? Math.Matrix4.identity;
    this._endMatrix   = this._endMover?.update(state, this)   ?? Math.Matrix4.identity;
  }
  ...
  set mover(Movers.Mover? mover) => this.startMover = mover;

  /// The mover to start position this light.
  Movers.Mover? get startMover => this._startMover;
  set startMover(Movers.Mover? mover) {
  ...
  set endMover(Movers.Mover? mover) {
