define(['dart_sdk', 'web/common/checkGroup', 'packages/ThreeDart/src/Core/Bindable', 'packages/ThreeDart/src/Math/Color3', 'packages/ThreeDart/src/Events/Changable'], function(dart_sdk, checkGroup, Bindable, Color3, Changable) {
  ...
  const tests__test018__test = Object.create(_root);
  ...
  tests__test018__test.main = function() {
    let page = new common__common.ShellPage.new("Test 018");
    page.addLargeCanvas("testCanvas");
    page.addPar(JSArrayOfString().of(["A test of the Material Lighting shader where a diffuse textue and ", "inverse diffuse texture are used. Grass is only shown in the dark. ", "Dirt is shown where the directional light is shining."]));
    ...
    let diffuse = td.textureLoader.load2DFromFile("../resources/Dirt.png");
    let invDiffuse = td.textureLoader.load2DFromFile("../resources/Grass.png");
    ...
    tech.diffuse.texture2D = diffuse;
    tech.invDiffuse.texture2D = invDiffuse;
    td.scene = (() => {
